...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#createActivity").click(function () {
			//初始化工作
			//重置
			$("#createActitvityForm").get(0).reset();
			//任意js代码
			//弹出创建市场活动的模态窗口
		...
		$(".mydate").datetimepicker({
			...
			autoclose:true,//设置选择完日期或者时间之后，否则自动关闭日历
			todayBtn:true,//设置是否显示"今天"按钮，默认是false
			clearBtn:true//设置是否显示"清空"按钮，默认是false
		});

		//当市场活动主页面加载完成，查询所有数据的第一页以及所有的总条数，默认显示10条
		...
		$("#editActiviBtn").click(function () {
			//收集参数
			//获取列表中被选中的checkbox
			var chedIds = $("#tBody input[type='checkbox']:checked");
			if(chedIds.size()==0){
				alert("请选择要修改的市场活动")
				return;
			}
			...
			var id = chedIds.val();
			// var id = chedIds.get(0).value;
			// var id = chedIds[0].value;
			//发送请求
			$.ajax({
		...
		$("#importActivityBtn").click(function () {
			...
			$.ajax({
				...
				contentType:false,//设置ajax向后台提交参数之前，是否把所有的参数统一按urlencoded编码:true--是，false--不
				type:'post',
				dataType:'json',
				success:function (data) {
					if(data.code =="1"){
	...
	function queryActivityByConditionForPage(pageNo,pageSize){
		...
		$.ajax({
			...
			success:function (data) {
				...
				//显示市场活动的列表
				//遍历activiyList,拼接所有行数据
				var htmlStr = "";
				$.each(data.activiyList,function (index,obj) {
				...
				$("#demo_pag1").bs_pagination({
					...
					showRowsPerPage: true,//是否显示“每页显示条数”部分。默认显示true--显示
		...
		<div class="modal-dialog" role="document" style="width: 85%;">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<form id="createActitvityForm" class="form-horizontal" role="form">
		...
		<div style="width: 100%; position: absolute;top: 5px; left: 10px;">
			...
			<div class="btn-toolbar" role="toolbar" style="background-color: #F7F7F7; height: 50px; position: relative;top: 5px;">
				<div class="btn-group" style="position: relative; top: 18%;">
				  ...
				  <button type="button" class="btn btn-default" id="editActiviBtn"><span class="glyphicon glyphicon-pencil"></span> 修改</button>
